...
<template>
  <div class="h-screen bg-[#FEFAF7]">
    ...
    <div class="flex flex-col items-center bg-[#FEFAF7]">
      <span
        class="text-2xl font-bold pb-10 md:pb-10 lg:pb-12 md:text-4xl lg:text-4xl"
        >晚到證明填寫</span
      >
      ...
      <div class="flex flex-col gap-10 justify-center items-start px-10">
        <div class="flex flex-col lg:pl-4 pl-2">
          <span class="font-semibold text-left text-lg lg:text-xl"
            >晚到事由：</span
          >
          <div class="p-1"></div>
          <textarea
            ...
            id="lateReason"
            name="lateReason"
            class="bg-[#fff] border border-black rounded-md w-56 lg:w-96 lg:h-32 p-3 lg:p-5"
      ...
      <div>
        ...
        >
          ...
          <button
            @click="updateLateReason(note1)"
            v-if="note1 && note1.trim() !== ''"
            class="bg-white px-3 py-2 border border-black rounded-full flex-none"
          >
            {{ note1 }}
          </button>
...
const props = defineProps(["onVnodeUnmounted"]);
const save = () => {
  const userData = {
...
const note1 = ref("");
onMounted(() => {
  note1.value = localStorage.getItem("晚到單常用事由");
});
...
onMounted(() => {
  ...
  if (user) {
    ...
    const lateReasonFromStorage = localStorage.getItem("晚到單事由");
    if (lateReasonFromStorage) {
      lateReason.value = lateReasonFromStorage;
    }
  }
});

const navigateToSet = () => {
  router.push("/commenreasonset");
};
const home = () => {
  router.push("/home");
};
const saveToLocalStorage = () => {
  localStorage.setItem("晚到單事由", lateReason.value);
};
</script>
